...
N_adrc = 2;
b0_adrc_roll =  15; % Guess based on bode plot
b0_adrc_yaw = 9;
% State space matrices of virtual plant
A_adrc = [zeros(N_adrc, 1)  eye(N_adrc, N_adrc) ;
            0       zeros(1, N_adrc)];
b_adrc_yaw = [zeros(N_adrc-1, 1);
             b0_adrc_yaw      ;
              1      ];

b_adrc_roll = [zeros(N_adrc-1, 1);
             b0_adrc_roll      ;
              1      ];

c_t_adrc = [1, zeros(1, N_adrc)];
%c = [1, 0, 0];
% Controller tuning based on desired time domain characteristics

T_settle_98 = 5; % Seconds

omega_cl = (1.9 + 2.09*N_adrc - 0.07*N_adrc^2) / T_settle_98;

k_adrc = ones(N_adrc,1); % controller_gains

for i=1:1:N_adrc

    k_adrc(i) = factorial(N_adrc) / (factorial(N_adrc-i + 1) * factorial(i-1)) * omega_cl^(N_adrc-i+1);
...
l_adrc = ones(N_adrc+1, 1); % Observer gains

for i = 1:1:N_adrc+1
    
    l_adrc(i) = factorial(N_adrc+1) / (factorial(N_adrc-i+1)*factorial(i)) * omega_O^i;
